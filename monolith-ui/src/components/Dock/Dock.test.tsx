...
describe("DockButton", () => {
  ...
  it("applies active class when active prop is true", () => {
    const { container } = render(
      <DockButton icon={<span>Icon</span>} active />,
    );
  ...
  it("uses tooltip or label for title attribute", () => {
    ...
    expect((container1.firstChild as HTMLElement).getAttribute("title")).toBe(
      "Label",
    );
    ...
    expect((container2.firstChild as HTMLElement).getAttribute("title")).toBe(
      "Tooltip",
    );
  });
});
