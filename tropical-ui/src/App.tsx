    ...
    <>
      ...
      <div className="doc-command-bar">
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '16px' }}>
          <span className="cmd-hint">
            <span className="cmd-key">CTRL</span> + <span className="cmd-key">K</span> SEARCH
          </span>
          <span className="cmd-hint">
            <span className="cmd-key">CTRL</span> + <span className="cmd-key">T</span> THEME
          </span>
        </div>
      </div>
      ...
      <nav className="doc-nav">
        <NavLink className="nav-logo" to="/">
          <div className="nav-logo-mark" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ width: '10px', height: '10px', background: 'var(--brand-primary)', borderRadius: '1px' }} />
          </div>
          <span style={{ fontWeight: 800, color: 'var(--ui-text-primary)', letterSpacing: '-0.02em' }}>Tropical</span>
            ...
            {Object.entries(BRAND_MAP).map(([key, { label, color }]) => (
              <div
                ...
                style={{
                  ...
                  background: color,
                  cursor: 'pointer',
                  border: brand === key ? '2px solid #fff' : '2px solid transparent',
                  boxShadow: brand === key ? '0 0 0 1px var(--ui-border-subtle)' : 'none',
      ...
      <Routes>
        <Route path="/" element={<DocLayout />}>
          ...
          <Route path="branding" element={<BrandTheming brand={brand} handleBrandClick={setBrand} />} />
